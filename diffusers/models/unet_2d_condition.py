...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
            ...
            heatmap_features=None,  # Mid-block features from heatmap (batch_size, 1280, 8, 8)
        ...
        if controlnet_cond is not None:
            if controlnet_interpolator is not None:
                ...
                # Apply the concept vector with attention if attentive heatmap features are provided
        ...
        if controlnet_cond_guidance is not None:
            ...
            if _nsize == 1:
                ...
                if controlnet_interpolator is not None:
                    sample[_nsize:_nsize + 1] = controlnet_interpolator(sample[_nsize:_nsize + 1],
                                                                        concept_vector_guidance)
                else:
                    ...
                    if heatmap_features is not None and heatmap_features.shape[0] > _nsize:
                        ...
                        attention_weights = torch.sigmoid(guidance_heatmap_features)
                        sample[_nsize:_nsize + 1] = sample[
                                                    _nsize:_nsize + 1] + concept_vector_guidance * attention_weights
                    else:
                        # Default behavior
                        sample[_nsize:_nsize + 1] += (1.0 * concept_vector_guidance)
            else:
